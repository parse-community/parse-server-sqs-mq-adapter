...
class Publisher {
  constructor(config) {
    if (!config.SQSEventEmitterMQOptions) {
      throw new Error('No SQSEventEmitterMQOptions found in config');
    }

    this.config = config.SQSEventEmitterMQOptions;
    const producer = SQSProducer.create(this.config);
